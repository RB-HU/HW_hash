...
static sample resample(URng& rng,
  ...
  for (size_t i = 0; i < resamples; ++i) {
    ...
    for (size_t s = 0; s < n; ++s) {
      resampled.push_back(first[static_cast<std::ptrdiff_t>(dist(rng))]);
    }
    const auto estimate = estimator(resampled.begin(), resampled.end());
...
static double standard_deviation(std::vector<double>::const_iterator first,
                                 std::vector<double>::const_iterator last) {
...
bool Config::listListeners() const {
...
bool Config::zeroTestsCountAsSuccess() const {
...
uint32_t Config::rngSeed() const {
  return m_data.rngSeed;
}
unsigned int Config::shardCount() const {
  return m_data.shardCount;
}
unsigned int Config::shardIndex() const {
...
bool Config::skipBenchmarks() const {
...
IEventListenerPtr prepareReporters(Config const* config) {
  if (Catch::getRegistryHub().getReporterRegistry().getListeners().empty() &&
    ...
    return createReporter(
        spec.name, ReporterConfig(config, makeStream(spec.outputFilename),
                                  spec.colourMode, spec.customOptions));
  }
  ...
  for (auto const& reporterSpec : config->getProcessedReporterSpecs()) {
    multi->addReporter(createReporter(
        reporterSpec.name,
        ReporterConfig(config, makeStream(reporterSpec.outputFilename),
                       reporterSpec.colourMode, reporterSpec.customOptions)));
  }
...
TestCaseInfo::TestCaseInfo(StringRef _className,
  ...
  for (size_t idx = 0; idx < originalTags.size(); ++idx) {
    ...
    if (c == ']') {
      CATCH_ENFORCE(inTag, "Found unmatched ']' while registering test case '"
                               << _nameAndTags.name << "' at " << _lineInfo);
  ...
  CATCH_ENFORCE(!inTag, "Found an unclosed tag while registering test case '"
                            << _nameAndTags.name << "' at " << _lineInfo);
...
IGeneratorTracker* createGeneratorTracker(StringRef generatorName,
  ...
  return getResultCapture().createGeneratorTracker(generatorName, lineInfo,
...
ColourMode ReporterConfig::colourMode() const {
  return m_colourMode;
}
...
void Context::setConfig(IConfig const* config) {
  m_config = config;
}
...
void listTags(IEventListener& reporter, IConfig const& config) {
  ...
  for (auto const& testCase : matchedTestCases) {
    for (auto const& tagName : testCase.getTestCaseInfo().tags) {
      ...
      if (it == tagCounts.end())
        it =
            tagCounts.insert(std::make_pair(tagName.original, TagInfo())).first;
...
void ReporterRegistry::registerReporter(std::string const& name,
  ...
  CATCH_ENFORCE(ret.second,
                "reporter using '" + name + "' as name was already registered");
}
...
Optional<ReporterSpec> parseReporterSpec(StringRef reporterSpec) {
  ...
  return ReporterSpec{CATCH_MOVE(parts[0]), CATCH_MOVE(outputFileName),
                      CATCH_MOVE(colourMode), CATCH_MOVE(kvPairs)};
}
...
Totals RunContext::runTest(TestCaseHandle const& testCase) {
  ...
  do {
    ...
    m_testCaseTracker = &SectionTracker::acquire(
        m_trackerContext,
        TestCaseTracking::NameAndLocationRef(testInfo.name, testInfo.lineInfo));
    ...
    auto statsForOneRun =
        TestCaseStats(testInfo, singleRunTotals, CATCH_MOVE(oneRunCout),
                      CATCH_MOVE(oneRunCerr), aborting());
  ...
  m_reporter->testCaseEnded(
      TestCaseStats(testInfo, deltaTotals, CATCH_MOVE(redirectedCout),
                    CATCH_MOVE(redirectedCerr), aborting()));
...
void RunContext::sectionEnded(SectionEndInfo&& endInfo) {
  ...
  m_reporter->sectionEnded(SectionStats(CATCH_MOVE(endInfo.sectionInfo),
                                        assertions, endInfo.durationInSeconds,
                                        missingAssertions));
  m_messages.clear();
  m_messageScopes.clear();
}
...
void RunContext::handleFatalErrorCondition(StringRef message) {
  ...
  Counts assertions;
  assertions.failed = 1;
  SectionStats testCaseSectionStats(CATCH_MOVE(testCaseSection), assertions, 0,
                                    false);
...
void RunContext::runCurrentTest(std::string& redirectedCout,
  ...
  SectionStats testCaseSectionStats(CATCH_MOVE(testCaseSection), assertions,
                                    duration, missingAssertions);
  m_reporter->sectionEnded(testCaseSectionStats);
}
...
Section::~Section() {
  if (m_sectionIncluded) {
    SectionEndInfo endInfo{CATCH_MOVE(m_info), m_assertions,
...
bool ITracker::isSectionTracker() const {
  return false;
}
bool ITracker::isGeneratorTracker() const {
  return false;
}
...
bool ITracker::hasStarted() const {
...
std::string WithinUlpsMatcher::describe() const {
  ...
  if (m_type == Detail::FloatingPointKind::Double) {
    ...
    write(ret, step(static_cast<float>(m_target),
                    -std::numeric_limits<float>::infinity(), m_ulps));
    ret << ", ";
    write(ret, step(static_cast<float>(m_target),
                    std::numeric_limits<float>::infinity(), m_ulps));
  }
...
void ReporterBase::listTests(std::vector<TestCaseHandle> const& tests) {
  defaultListTests(m_stream, m_colour.get(), tests, m_config->hasTestFilters(),
                   m_config->verbosity());
}
...
void ConsoleReporter::assertionEnded(AssertionStats const& _assertionStats) {
  ...
  ConsoleAssertionPrinter printer(m_stream, _assertionStats, m_colour.get(),
                                  includeResults);
  printer.print();
  m_stream << '\n' << std::flush;
}
