...
// in the internal header HashTable_priv.hpp.  This lets us define a pointer
...
// that frees the keys and corresponding values.  The pointed-to function is
// invoked once for each key-value-pair in the HashTable.
typedef void (*KeyValueFreeFnPtr)(HTKeyValue_t value);

// When creating a HashTable, the user will need to supply a function
// that is used to compare two keys. This is because just comparing the
// Hash value is not enough to tell whether two key value pairs have equal
// keys or not. The pointed to function should return ture iff the two keys
// are structurally equal.
typedef bool (*KeyCmpFnPtr)(HTKey_t, HTKey_t);
...
// - key_compare_function: a function pointer to compare two keys.
...
//
// Arguments:
// - table: the HashTable to deallocate. It is unsafe to use table
...
// - hash: the hash of the key to look up.
...
//
// Arguments:
// - iter: the iterator to move.  Must be non-nullptr.
//
// Returns:
...
//
// Arguments:
// - iter: the iterator to fetch the (key,value) from.  Must be non-nullptr.
